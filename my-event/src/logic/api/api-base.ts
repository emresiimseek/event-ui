...
class ApiBase {
    ...
    async postRequest(url: string, userDto: UserAuthenticationDto): Promise<AxiosResponse> {
        return await (this.conduitApi.post(url, userDto,).then(response => response).catch(error => error.response));
    }
}
